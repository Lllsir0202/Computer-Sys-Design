...
static inline make_DopHelper(SI) {
  ...
  // bug！！！
  // 如果直接转换，由于这里可能只有1个byte，直接转换不会处理，所以我们不能这么做 -> bingo
...
make_DHelper(J) {
  ...
  Log("id_dest->simm is %d", id_dest->simm);
  Log("eip is %x", *eip);
  decoding.jmp_eip = ((int32_t)(id_dest->simm)) + *eip;
  Log("decoding.jmp_eip is %x", decoding.jmp_eip);
}
